...
<template>
  <section class="h-full grid grid-cols-[20rem_1fr] grid-rows-[auto_1fr]">
    <div class="col-start-1 row-span-full bg-page-light-bg p-8">
      <CarPreferencesFilterPanel />
    </div>
    <div class="col-start-2 justify-self-start ml-8 bg-page-light-bg">
    ...
    <div class="m-8">
      <ul class="grid grid-cols-[repeat(auto-fill,_minmax(20rem,_1fr))] gap-8">
        <li v-for="(car, index) in cars" :key="index">
          <CarRentCard :car="car" card-bg="page-light-bg" />
        </li>
      </ul>
    </div>
  </section>
